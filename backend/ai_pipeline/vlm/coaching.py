...
def _build_universal_prompt(
    ...
    sys_prompt = (
        ...
        "════════ NON-RACQUET ACTIVITIES (gym / strength / calisthenics / "
        "swimming / running / etc.) ════════\n"
        "If this is NOT a racquet sport, this is your guidance instead:\n"
        "• GYM / STRENGTH / CALISTHENICS (push-ups, pull-ups, squats, "
        "lunges, planks, tricep extensions, bicep curls, bench press, "
        "deadlifts, shoulder press, rows, etc.): each EVENT is ONE REP of "
        "the exercise. `shot_category` = the exercise name in snake_case "
        "(e.g. 'push_up', 'tricep_extension', 'squat', 'lunge', "
        "'bicep_curl', 'deadlift'). `shot_label` describes that rep "
        "('Push-up 3 — hips sagging', 'Tricep extension — full lockout'). "
        "Read the BODY POSITION to name the exercise correctly, do not "
        "guess: push-up = body roughly horizontal, hands on floor, elbows "
        "bend then arms push up; lunge = standing, one leg steps forward "
        "and the back knee drops, torso upright; squat = both feet planted, "
        "hips drop straight down; tricep extension = arms overhead or elbow "
        "fixed, forearm extends; plank = static hold. Do NOT confuse a "
        "push-up (horizontal) with a lunge (standing) — they look nothing "
        "alike.\n"
        "• For these activities there is no opponent, no court, no serve. "
        "intent = 'neutral', outcome = 'continued_rally' for a clean rep / "
        "'unknown' otherwise. Count every visible rep as its own event.\n"
        "• Coaching should focus on form: range of motion, joint alignment, "
        "tempo, core stability, full lockout/extension.\n"
