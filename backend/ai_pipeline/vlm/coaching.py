...
def _build_universal_prompt(
    ...
    sys_prompt = (
        ...
        "════════ RACQUET SPORTS ONLY (badminton, tennis, table "
        "tennis, pickleball, squash) ════════\n"
        "The next three rule blocks (SERVE / FLAT-EXCHANGE / RALLY "
        "VARIETY) apply ONLY when you identified a RACQUET sport in Step 1. "
        "If the video is anything else — gym/strength training, swimming, "
        "running, gymnastics, combat, etc. — IGNORE these three blocks "
        "entirely and use the NON-RACQUET section further below. Never let "
        "racquet vocabulary (serve, smash, drive, clear, net shot) leak "
        "into a non-racquet activity, and never assume the sport is "
        "badminton.\n\n"
        ...
        "If this is NOT a racquet sport, this is your guidance instead. You "
        "KNOW these activities well — name what you actually see using your "
        "OWN knowledge. Do NOT pick from a short menu and do NOT borrow "
        "racquet vocabulary (no serve, smash, drive, rally, court, ball "
        "trajectory).\n\n"
        "• GYM / STRENGTH / CALISTHENICS: each EVENT is ONE REP. "
        "`shot_category` = the exercise's standard name in snake_case "
        "(whatever it actually is — e.g. tricep_rope_pushdown, lat_pulldown, "
        "bench_press, goblet_squat — use the real name from your knowledge, "
        "not a fixed list). `shot_label` describes that rep ('Rep 3 — full "
        "lockout'). intent='neutral'; outcome='continued_rally' for a clean "
        "rep else 'unknown'. Coach FORM: range of motion, joint alignment, "
        "tempo, spine/core stability, lockout.\n"
        "  Before you name the exercise, READ THE CLIP (don't name it by "
        "vibe — the first guess is often wrong): (a) EQUIPMENT — bodyweight, "
        "barbell, dumbbell, cable stack/pulley, machine, kettlebell? (b) "
        "BODY ORIENTATION — standing, seated, lying, bent-over, horizontal? "
        "(c) WHICH JOINTS MOVE — if the upper arms/thighs stay fixed and "
        "only ONE joint moves it is an ISOLATION lift (e.g. only the elbow "
        "extending against a cable = a triceps move, NOT a row or a "
        "pulldown); if the whole limb + hips/shoulders drive together it is "
        "a COMPOUND lift. (d) LOAD DIRECTION — pushing the weight away vs "
        "pulling it toward you. Let those four reads pick the name.\n"
        "• CONTINUOUS activities (running, cycling, swimming laps, rowing): "
        "there are no discrete 'reps'. Segment by stroke/pedal CYCLE or by "
        "notable form moments instead, and coach posture, cadence, "
        "efficiency.\n"
        "• OTHER skill sports (golf, bowling, etc.): each EVENT is one "
        "attempt/swing; name it normally (golf_drive, putt…) and coach the "
        "mechanics.\n"
        "• IF UNSURE of the exact name, give the movement/pattern you ARE "
        "sure of and set confidence < 0.6 — never swap in a different "
        "exercise just to sound confident. A vague-but-correct label beats "
        "a precise-but-wrong one.\n"
        "════════ END NON-RACQUET SECTION ════════\n\n"
        + (
            (
