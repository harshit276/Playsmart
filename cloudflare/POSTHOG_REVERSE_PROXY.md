...
Serves PostHog from a first-party subdomain (`ph.athlyticai.com`) so ad-blockers
...
**Prerequisite:** `athlyticai.com` must be managed in Cloudflare DNS (it is — the
...
## Step 2 — Bind the subdomain `ph.athlyticai.com`

1. In the Worker → **Settings** → **Domains & Routes** → **Add** → **Custom Domain**.
2. Enter `ph.athlyticai.com` → **Add domain**.
...
```
https://ph.athlyticai.com/static/array.js
```
...
> **Order matters:** do NOT change `api_host` to `ph.athlyticai.com` until the
...
- The PostHog snippet derives its asset URL from `api_host`, so with
  `api_host: https://ph.athlyticai.com` the bundle loads from
  `ph.athlyticai.com/static/array.js`, which the Worker routes to `eu-assets`.
...
- When the in-progress `src/lib/analytics.js` (env-driven) becomes the single init
  path, set `REACT_APP_POSTHOG_HOST=https://ph.athlyticai.com` and remove the
