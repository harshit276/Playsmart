...
import { Zap, Mail, ArrowLeft, Clock, RefreshCw } from "lucide-react";
...
export default function AuthPage() {
  ...
  const handleVerifyOTP = useCallback(async (otpValue) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/auth/verify-otp", { email: email.trim().toLowerCase(), otp: code });
            ...
            >
              ...
              <form onSubmit={handleSendOTP} className="space-y-6">
                <div>
                  ...
                  <p className="text-zinc-400 text-sm">Enter your email to receive a one-time password.</p>
                </div>
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-zinc-400 text-sm">Enter the 6-digit code sent to <span className="text-lime-400">{email}</span></p>
                </div>
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" onClick={() => { setStep("email"); setOtp(""); setSecondsLeft(0); }}
                    className="flex-1 text-zinc-500 hover:text-zinc-300 text-sm" data-testid="change-email-btn">
