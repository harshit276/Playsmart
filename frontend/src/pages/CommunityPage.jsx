...
export default function CommunityPage() {
  ...
  useEffect(() => { document.title = "Community | Formanti"; }, []);
            ...
            <div className="space-y-3">
              {players.length === 0 ? (
                <EmptyState
                  ...
                  body="Share Formanti with friends — every new signup grows the local community."
                />
              ) : (
                players.map((p, i) => (
...
function GameCard({ game, userId, onJoin, onLeave, delay = 0 }) {
  ...
  const shareWhatsApp = () => {
    const text =
      ...
      `Join via Formanti: ${window.location.origin}/community`;
